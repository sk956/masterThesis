...
line <- ggplot() + 
  geom_line(data = tableP1avg, aes(x = min_upgrade, y = average_e), color = 'blue') +
  xlab('minute') + 
  ylab('average_emotion')
...
model <- lm(average_e ~min_upgrade, data = tableP1avg)


line2 <- ggplot(tableP1avg, aes(x = min_upgrade, y = average_e)) + 
  geom_point() 
